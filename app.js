...
async function initializeGapiClient() {
  try {
    ...
    authStatus.textContent = 'Setup Error: ' + (err.details || err.message || JSON.stringify(err));
...
function updateNavIndicator(hash) {
  ...
  if (activeLink) {
    ...
    if (isInitialRender) {
      ...
      // Wait for the browser to fully paint the nav before measuring
...
function renderFeed() {
  ...
  entries.forEach(item => {
    ...
    // Prevent mobile long-press context menu popping up
