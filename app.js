...
let searchQuery = ''; 
let selectedSearchTag = null; // NEW: Track the currently selected filter tag
...
const views = {
  ...
  '/search': document.getElementById('view-search'), // NEW: Search View
  '/add': document.getElementById('view-add'),
  '/profile': document.getElementById('view-profile')
};
const feedGrid = document.getElementById('feed-grid');
const searchFeedGrid = document.getElementById('search-feed-grid'); // NEW
const searchTagsContainer = document.getElementById('search-tags-container'); // NEW
...
function handleRoute(noAnimate = false) {
  ...
  // Recalculate masonry spans in case the newly visible view has differently sized elements
...
function setMasonrySpans() {
  ...
  document.querySelectorAll('.masonry-item').forEach(item => {
    ...
    // Only calculate if the element is actually visible (prevents errors on hidden tabs)
    if (contentHeight > 0) {
...
// Extract unique tags from all user entries
...
// Render dynamic filter tags in the search view
  ...
  tags.forEach(tag => {
    ...
    btn.onclick = () => {
      if (selectedSearchTag === tag) {
        selectedSearchTag = null; // Toggle off if already selected
      } else {
...
// HELPER: Creates the DOM element for an individual entry
...
// Renders the main Home view (unfiltered)
function renderFeed() {
  feedGrid.innerHTML = '';
  ...
  // Ensure search updates dynamically if data changes
  renderSearchTags();
  renderSearchFeed();
...
// Renders the Search view (filtered by text and/or tag)
...
function applySelectionStyles() {
  ...
  // This smoothly targets ALL cards across Home and Search views
...
handleRoute(true);const detailContent = document.getElementById('detail-content');
const btnSheetClose = document.getElementById('btn-sheet-close');
